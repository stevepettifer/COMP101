...
This should respond with \fname{/usr/bin/mutt}, telling us that the \fname{mutt} command has been put in the \fname{/usr/bin} directory on our system. Recall that  in the last sessions we looked at the contents of the \fname{/bin} directory that contained essential low-level commands such as \ttout{ls}; the directory \fname{/usr/bin} contains commands that aren't quite as essential, but have been installed for the users' benefit (i.e. the system would boot and work without them, but it just wouldn't be very useful.) 
...
Here are some other very useful command line tricks for you to try out (give them each a go now so that you remember them for later):
...
to see the result. Okay, so this probably won't replace your favourite weather web page or app, but it'days early days yet! 
...
As well as GNOME and KDE, which both follow fairly similar interaction paradigms to the graphical environments of other operating systems, there are a plethora of alternative graphical interfaces for Linux. 

Use the table that follows to explore one or more of the alternative window managers by using the value from the `Command' column as the first command-line argument to \ttout{csstartx}. 
