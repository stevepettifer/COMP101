...
%%% BEGIN CONSOLE MODE SESSION
 ...
 You are now logged in to a PC that is part of our School Linux network. Type \cmnd{pwd}{pwd} to find out which directory you are in. It should be something like \ttout{/home/mbaXXXXX}, where the part after the \ttout{/} is your username. This is your home directory, which is not actually stored on the desktop PC but on a central fileserver. This means that, whichever machine you use in the lab, you will always see the same home filestore. 

 The environment you are now in is known as \concept{console mode}. This is a way of interacting with  computer via a screen containing only text, without the now familiar windows and images. All interaction is done using a \concept{command line interface} (CLI), typing commands into a program known as a \concept{shell}. A useful summary of the history of the term console can be found at \urlnop{www.linfo.org/console.html}. Later we will be using a graphical environment, but for now we will stick to the console and start off by reading mail.
...
This should respond with \fname{/usr/bin/mutt}, telling us that the \fname{mutt} command has been put in the \fname{/usr/bin} directory on our system. Recall that  in the last sessions we looked at the contents of the \fname{/bin} directory that contained essential  commands such as \ttout{ls}; the directory \fname{/usr/bin} contains commands that aren't quite as essential, but have been installed for the users' benefit (i.e. the system would boot and work without them, but it just wouldn't be very useful). 
...
and you should find that a different graphical desktop environment, KDE, starts up. Experiment with this for a minute or two just to get a feel for what it's like, and then quit back to the console prompt.
...
Use the table that follows to explore some of these  by using the value from the `Command' column as the first command-line argument to \ttout{csstartx}. 
...
AnotherLevelUp & \ttout{ALU} & A customised version of fvwm developed  here in CS by John, and still his favourite desktop environment (may contain bright colours) \textbf{Click the left mouse button.}\\
Fluxbox & \ttout{fluxbox} & A lean but highly customisable window manager \textbf{Click the right mouse button}.\\
Openbox & \ttout{openbox} & Similar to Fluxbox; lean and highly configurable. \textbf{Click the right mouse button.} \\
...
Xmonad & \ttout{xmonad} & Another very very lean tiling window manager, similar to Awesome.  
\end{tabular}
...
Start up the \cmnd{nano}{nano} editor, and use it to add a new line at the end of your \fname{.bash\_profile} that reads 
...
Now log out from console mode, and log back in again; you should find that the graphical environment fires up automatically. 

Next, quit out of whichever graphical environment you've chosen and\ldots oh dear, you're back at the console mode command prompt, rather than logged out completely. 
...
At the console mode command prompt, type the following:
...
Now log out (either type \texttt{logout} (or press \ctrl{d} to tell the shell that its input has ended), and log back in again. If all has gone to plan then you should see the graphical environment fire up automatically; and when you quit the graphical environment, you should be returned to the Linux login prompt. 
...
When you started the ARCADE client or used the \fname{csstartx} command, you had to prefix these with their full pathname, which is \fname{/opt/teaching/bin/}. The directory
...
This won't have any effect until you logout and login again. So do that now and try starting \ttout{arcade}. Run \ttout{echo \$PATH} and you will see the new value, with your own \ttout{bin} directory at the end, preceded by the three \ttout{/opt} directories.
