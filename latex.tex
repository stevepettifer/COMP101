...
\end{figure}
\end{verbatim}
which we used to create Figure~\ref{figure:menandcheeses}. You can
...
\verb|\begin...\end| code `wraps' up whatever picture you're
...
make the most sensible choice. It's possible to exercise finer control
over figure placement, but that's beyond the scope of this exercise.

The \verb+\includegraphics+ command is not built in to core \LaTeX\
but is in an additional \concept{package}, which needs to be
explicitly loaded. We load this package by this using the command
...
\verb+\begin{document}+. There are hundreds of such packages that
  extend \LaTeX{}'s functionality; these lab scripts use over 30 of them.
...
In the \LaTeX\ code,  \verb|\left[| means ``big opening square bracket please'';  \verb|{cccc}| means ``an array with 4 columns please, with the items in each column centred"; \verb|&| means ``start a new column''; and \verb|\\| means ``start a new row''. You'll notice that \LaTeX\ knows about  greek letters; it knows about most standard maths symbols too, and also they ways  they're usually used.
