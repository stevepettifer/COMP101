...
Let's first see what stuff we already have on our Pi. The \cmnd{ls}{ls} command lists files and directories. Type it now, and you should see that two directories/folders have already been created for you, one called \fname{python\_games} and the other called \fname{Desktop}. 

\begin{ttoutenv}
$ ls
Desktop python\_games
\end{ttoutenv}
...
We now need to get back to our home directory. You should by now be able to think of There are many ways of doing this, including the following:
...
Rather than browsing the web in the normal way, we sometimes want to just obtain a copy of file; we can do this the command  \cmnd{curl}{curl}. First, use \cmnd{cd}{cd} to change to your home directory if you're not there already, then
...
\\
making sure you save it in your home directory. Remember that, by default, \ttout{curl} will just output whatever it has fetched to the screen. Use the option you learned about earlier to save it to a file called \fname{quake.tar.gz}.
...
This command executes the file \fname{ioquake3.arm} which is in the current directory. After the startup screen, the game will ask you for a code, but you can just skip this and use the mouse to select the play option. The rest, we're sure, you can figure out for yourself. 
