...
Although it is actually possible to browse the web without a conventional web browser on the Pi (we'll experiment in another lab session because there are interesting lessons to be learned by doing so), there's an easier way to interact with the web to get the file that we need, using a command called \cmnd{curl}{curl}. First, use \cmnd{cd}{cd} to change to your home directory if you're not there already, then
