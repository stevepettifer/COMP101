...
to run forever (or at least until you logout). In this situation you can, of course quit the clock, using \ctrl{c}  in the
 \cmdname{xterm} window. However, if you want to keep the clock running and still get on with other things in the same terminal window you can just add an ampersand (\ttout{\&}) on the end of a command, so in this case,
\begin{ttoutenv}
\$ xclock \&
\end{ttoutenv} 
