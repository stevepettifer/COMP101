...
\begin{itemize}
\item Edit the line that starts \ttout{set my\_user\_name} so include your University email address.
...
\item Edit the line that starts \ttout{set realname} to include your real name, in whatever way you want it to appear in outgoing emails.
\end{itemize}
...
\ttout{mv} may seem like an odd name for a command that is used to rename a file, but it actually has a number of uses, including moving a file from one part of the file hierarchy to another. You'll see more examples of the in the next lab session.
...
The way to make the change permanent is to modify your  \fname{.bash\_profile} file. While we are doing this we will also add another useful directory to our PATH, namely \fname{/opt/common/bin}, another shared directory containing useful stuff.
...
Rather than explain the Thunderbird configuration here we refer you \href{http://wiki.manchester.ac.uk/compsci/index.php/How_to_set_up_Office_365_Mail_in_Thunderbird}{to an illustrated guide to this process}, written by a fellow student.

Follow all three steps in the process described here.
