\chapter{Using the Linux desktop in CS: DRAFT VERSION}
...
Much of what you do in these and future labs will involve using terminal windows in one way or another.
...
The way to make the change permanent is to modify your  \fname{.bash\_profile} file. While we are doing this we will also add another useful direcotory to our PATH, namely \fname{/opt/common/bin}, another shared directory containing useful stuff.

Use \ttout{gedit} to modify  \fname{.bash\_profile} by adding the following line \emph{after the existing line starting with \ttout{PATH} but before the line containing \ttout{exec startx}}.
...
\emph{Note that the characters surrounding \ttout{tty} are backquotes, not forward quotes. This character can be found on the keyboard of lab machines in the top left hand corner, just below \ttout{Esc}.}
