...
Windows. This is for flexibility -- the labs for most course units actually use
...
sure your PC is running Windows! Have a look. If it is, and the screen looks like~\ref{figure:welc-win}, then you  you can move on to the end of this subsection and login.

If it isn't in Windows, and it probably isn't, we need to reboot the PC. To do
this, find the 3 keys labelled `ctrl', `alt' and `delete' (see Figure~\ref{figure:welc-keyboard}) and press
all them simultaneously.

\begin{figure}
\centerline{\includegraphics[width=15cm]{images/TH-ctrl-alt-del}}
...
reading \ttout{Windows 7}, and press the enter key. After a short while, the Windows7
...
free PC into Windows  and use it at any time unless you are in a lab that requires the use of Linux. Next, we're going to take a first look at Linux.
...
the up/down arrow keys to select \ttout{EPS Linux (Scientific Linux)}. Linux will now start, and
after a short while you should see a black screen, with a login prompt
in the lower left. 

Enter your username and password -- the same ones you used when
...
Figure~\ref{figure:welc-linux}, but with your username instead of the one
