...
The command to change the password is \totype{passwd}; typing this at the command prompt and entering your current password (i.e. `raspberry') and new password (twice, to be sure), should look like this:
...
\begin{note}
Get them to log out and back in again to practise their password. Can we say something about ctrl D here as well as logout?
\end{note}
...
When we're using a command-line prompt, we have the notion of of \wikipedia{http://en.wikipedia.org/wiki/Working_directory}{current working directory} which is the directory that we're currently `in'. Generally, commands we issue that don't specify another directory are assumed to refer to the current working directory; so \totype{ls} on its own really meant `run the list command on my current working directory'. 

\begin{note}
Just talk about LS rather than generalising about assuming directories.
\end{note}
...
\noindent and then use the \totype{ls} command to list its contents. You should see a long list of files: some are programs written in the Python programming language (these files end in \fname{.py}), others are images or sounds used by those programs (ending in \fname{.png} or \fname{.wav}). At the prompt type:
...
\begin{note}
Can we move this section a bit earlier, e.g. before the snake game gets run
\end{note}
...
\begin{note}
Move this breakout to somewhere where they create some files
\end{note}
...
So let's see where we are in the Pi's filesystem at the moment. Assuming you're following these instructions properly you should still be in the \fname{python\_games} directory (check the command prompt to confirm this is the case). To go back to our home directory, we can use the \totype{cd} command without any arguments:
\begin{ttoutenv}
$ cd
\end{ttoutenv}
...
The system we want to install to play this game is called \textit{frotz} (to learn why, you'll have to play the game a bit, or at least look it up on Google) Let's try running the \totype{apt-get} command \textit{without} having gained superuser privilege first. Try typing:
...
\noindent \urlnop{studentnet.cs.manchester.ac.uk/ugt/2013/COMP10120/files/Advent.z5} 
...
\\
\urlnop{studentnet.cs.manchester.ac.uk/ugt/2013/COMP10120/files/quake3.tar.gz}
\\
...
\noindent When tar has finished working you're presented again with the command prompt. Use \totype{ls} to confirm that you now have a directory called \fname{quake3} in your home directory.  You might also want to plug some headphones into the Pi at this point too, if you happen to have a pair with you. 
