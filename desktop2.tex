...
Firstly, you will recall three directories of particular interest.
...
be referenced using a single dot(\ttout{.}), as in when running the
\ttout{./quake3} program.
...
And you should recall the difference between an \concept{absolute path}---one
...
current working directory. Relatedly, you have met the `double dot' notation
(\ttout{..}) to mean `go up one level', as in \ttout{ls ..} or perhaps more
often \ttout{cd ..}, or even \ttout{cd ../x/y} etc..

Speaking of \cmdname{ls}, you will from time to time need to use its
\ttout{-a} switch argument to ask it to show files beginning with a dot,
and/or \ttout{-l} to make it show details about the files.

\begin{note}
Now done above, I hope!

  \begin{itemize}
  \item   Remind 3 dirs, HOME = ~, /, Current = ., pwd
  \item Abs vs relative pathname
  \item ..?
  \item  ls -a, -l reminder
  \end{itemize}
...
We start by using the \cmdname{mkdir} (\concept{make directory}) command
...
Now go into your \ttout{INTRO} directory and make directories for each of the
\crsname{} exercises.
...
Now you should make a directory, in your home directory, for the
COMP16121 course unit with sub-directories for each of the 10
exercises associated with that course. You \emph{must} use the same
convention as above: capital `COMP16121' and lower case `ex1',
etc..
...
Try the following sequence of \cmdname{cd}s, checking where you are by running \cmdname{pwd} 
after each one, and make sure you understand what is going on!
...
\begin{linux}{fortune}
  ...
  \wikipedia{Fortune_(Unix)}{fortune} is a simple program that displays a random message from a database of (supposedly) humorous quotations, such as those that can be found in the US in \wikipedia{Fortune_Cookies}{fortune cookies} (hence the name), quotations from famous people, jokes, or poetry.
...
\end{ttoutenv}
%
Our previous use of \ttout{*} has always been in conjunction with \cmnd{ls}{ls} which might have led you to think that the wild card was being expanded by \cmnd{ls}{ls}. In fact the expansion is done by the \emph{shell}, \cmnd{bash}{bash}.

One place where you have take care with wild cards is provided by the dotfiles; i.e. those
whose names begin with \ttout{.}. The asterisk will not match a
...
\ttout{\tilde/bin/wisecow}, e.g. run

\begin{ttoutenv}
\$ mkdir \tilde/bin
\$ nano \tilde/bin/wisecow
\end{ttoutenv}

and put into it the following text.
...
though they had been type into bash in the `normal' way.
...
Hopefully you noticed that failed! Before the operating system will believe us
that this really is a program, we need to give the file \concept{execute
...
Notice one of the directories listed is your very own \ttout{\tilde/bin} -- this
...
Hopefully done above?
  Use fortune | cowsay as the shell script 
\end{note}
...
\end{ttoutenv}
%
to print the given files. The printing service we use is the University \concept{Pull} printing service, which allows you to collect your printing at \emph{any} University pull printer. This is described in more detail at \href{http://www.itservices.manchester.ac.uk/printing/pullprinting/}{www.itservices.manchester.ac.uk/printing/pullprinting}.
...
\cmdname{lpr}. So, you shall print out just the first 50 lines of
...
It is common across the University for students to pay for your their own printing, but the Computer Science policy is to credit your printing account with enough for you to print all the material needed for your courses (with some to spare) without having to pay for anything. For administrative reasons, your initial credit will be £4, and this will be topped-up during the year. (The first top-up is likely to occur during Reading Week.)
...
\begin{note}
  Turn this into a driving test, email to tutor
Hopefully done below.
\end{note}

Here are a number of exercises to experiment with if you finish everything
else. More details of the relevant commands can be found by using
\cmdname{man}. We would like you to email your answers to your personal tutor
...
\item Look at the \cmdname{man} entry for  \cmdname{rm} and find out
  ...
  Question: What would it do in your home directory? What would it do if the
...
in the dictionary which contain the string `red'.
  ...
  Question: what was the command you used to do this? (Please don't email your
...
\item 
  ...
  Question: what was the command you used to do this? What was the answer?
...
\subsection{Then end of the beginning}

We have now come to the end of the introductory labs, and what follows in the
various courses are real ones that will be marked and which count! Especially
if you are new to Unix/Linux it is a good idea to spend a little time skimming
through what we have already done in order to ready yourself. Most
importantly: if you are feeling overwhelmed or even just a little concerned
about your understanding of what you have met so far, please do not suffer in
silence: speak to your personal tutor, or email one or all of your team of
torturers! And then we can help you.

\begin{tabular}{ll}
Steve & srp@cs.man.ac.uk \\
Graham & gdg@cs.man.ac.uk \\
Toby & tljh@cs.man.ac.uk \\
John & jtl@cs.man.ac.uk \\
\end{tabular}
